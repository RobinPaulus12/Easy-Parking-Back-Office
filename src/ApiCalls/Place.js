...
export const usePlace = () => {
  ...
  const addPlace = ({ arrival_time, departure_time, fk_parking }) => {
    return axios({
      method: "post",
      url: `${REACT_APP_API_URL}/place/`,
      data: { arrival_time, departure_time, fk_parking },
      headers: { Authorization: `Bearer ${token}` },
    });
  };

  const updatePlace = ({ place_id, arrival_time, departure_time, fk_parking }) => {
    return axios({
      method: "patch",
      url: `${REACT_APP_API_URL}/place/`,
      data: { place_id, arrival_time, departure_time, fk_parking },
      headers: { Authorization: `Bearer ${token}` },
    });
  };
  ...
  const addPlaceWithParking = ({
    ...
    return axios({
      ...
      data: {
        place: { arrival_time, departure_time },
        parking: { name, coordinates, places, telephone, opening, place_width, fk_locality },
      },
      headers: { Authorization: `Bearer ${token}` },
    });
  };
